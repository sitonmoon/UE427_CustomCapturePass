...
bool FMaterial::UsesCustomCapture_GameThread() const
{
	return GameThreadShaderMap.GetReference() ? (GameThreadShaderMap->UsesSceneTexture(PPI_CustomCapture)) : false;
}
...
void FMaterialAttributeDefinitionMap::InitializeAttributeMap()
{
	...
	// Style attributes
	Add(FGuid(0x5BF6BA9B, 0x51CE4082, 0x9D777BEE, 0xF4F72C44), TEXT("RampColor"),			MP_RampColor,			MCT_Float3,	FVector4(1,1,1,0),	SF_Pixel);
...
FText FMaterialAttributeDefinitionMap::GetAttributeOverrideForMaterial(const FGuid& AttributeID, UMaterial* Material)
{
	...
	case MP_CustomData0:	
		...
		CustomPinNames.Add({MSM_Style, "Ramp Mask"});
	...
	case MP_CustomData1:
		...
		CustomPinNames.Add({MSM_Style, "Intensity Mask"});
	...
	case MP_RampColor:
		return LOCTEXT("RampColor", "Ramp Color");
