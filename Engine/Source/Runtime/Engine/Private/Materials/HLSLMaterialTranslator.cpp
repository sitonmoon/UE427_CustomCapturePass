...
,	TargetPlatform(InTargetPlatform)
{
	...
	SharedPixelProperties[MP_RampColor] = false;
...
bool FHLSLMaterialTranslator::Translate()
{
	STAT(double HLSLTranslateTime = 0);
	{
		...
		// style
		Chunk[MP_RampColor]						= Material->CompilePropertyAndSetMaterialProperty(MP_RampColor	,this);
...
#ifdef ENGINE_CUSTOM_SHADING_MODEL_STYLE
		if (ShadingModels.HasShadingModel(MSM_Style))
		{
			OutEnvironment.SetDefine(TEXT("MATERIAL_SHADINGMODEL_STYLE"), TEXT("1"));
			NumSetMaterials++;
		}
#endif
...
{	
	...
	// style
	LazyPrintf.PushParam(*GenerateFunctionCode(MP_RampColor));
